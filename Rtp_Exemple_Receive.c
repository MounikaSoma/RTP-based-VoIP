...
void us_event(struct us *us, int cid, int *len,int ac, char **av)
{
    ...
    // initialising (creating) a playback stream
    ...
    if (FD_ISSET(srv->fd, &(us->fdset)))
    {
        ...
        // decodes the msg and adds the client to the list in the server
    ...
    finish:
    // removes data already in the buffer and start fresh when next data comes coz the buffer needs to be empty for the next data.
...
// start the server and start listening code
void us_serve(struct us *us,int ac, char **av) //Create cid here
{
    ...
    newTime.it_interval.tv_nsec = 500;  // wait for 500 nano seconds and then receive the next packet. If there is no such limit, all data was not being read.
